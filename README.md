...
### Unlocking the Research Console

1. Expand the **“🔐 Researcher console”** at the top of the app.
2. Enter the `ADMIN_KEY` to enable admin mode.
3. Set the key via Streamlit secrets (`.streamlit/secrets.toml`) or the environment:

```bash
export ADMIN_KEY="super-secret-passphrase"
streamlit run streamlit_app.py
```

Admin mode reveals the sidebar, dataset upload controls, the research dashboard toggle, and one-click study analysis.
...
├── streamlit_app.py             # Web application (participant + admin)
...
└── study_outputs/               # Generated by analyze_study.py & research dashboard
