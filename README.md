...
- **Automatic Logging** - JSON session data for each participant
...
- **Data Visualization** - Boxplots, bar charts, distributions
- **Report Generation** - Automatic markdown reports and CSV exports
...
git clone https://github.com/yourusername/stress-risk-assessment.git
cd stress-risk-assessment

# Install dependencies
pip install pandas numpy scikit-learn lightgbm shap streamlit plotly
...
mkdir data artifacts study_logs
...
```bash
streamlit run streamlit_app_stress.py
```
...
├── train_stress.py              # Model training pipeline
├── streamlit_app_stress.py      # Web application
├── analyze_study.py             # Analysis and visualization
├── README.md                    # This file
│
├── data/
...
Session automatically saved to `study_logs/[participant_id]_[group]_[timestamp].json`
...
### Web Application (`streamlit_app_stress.py`)

1. **Group Selection**: Assign participant to G1 or G2
2. **Questionnaire**: Collect 26 health/lifestyle metrics
3. **Prediction**: Get risk score and uncertainty
4. **Display**: Show results based on group
5. **Plan Generation**: Create personalized stress management plan
6. **Survey**: Collect feedback and ratings
7. **Logging**: Save session data as JSON
